...
func init() {
	android.RegisterModuleType("dirty_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = dirtyExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = dirtyExpandVariables(ctx, dep_file)
	...
	cmd := dirtyExpandVariables(ctx, String(g.properties.Cmd))
